...
public class ItemsProvider extends ContentProvider {
	...
	private static final int ITEMS__SERVER_ID = 2;
	...
	private static UriMatcher buildUriMatcher() {
        ...
        matcher.addURI(authority, "items/by_server_id/*", ITEMS__SERVER_ID);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (match) {
			case ITEMS: {
                long _id = db.insertWithOnConflict(Tables.ITEMS, null, values, SQLiteDatabase.CONFLICT_IGNORE);
                if (_id == -1) {
                    // insert failed - we do a replace
                    // assuming that the server_id has a conflict
                    String serverId = values.getAsString(ItemsContract.ItemsColumns.SERVER_ID);
                    int count = update(
                            uri,
                            values,
                            ...
                            new String[]{serverId});
                    if (count < 1) {
                        throw new SQLException("Could not insert / replace row");
                    }
                    Cursor cursor = db.query(
                            Tables.ITEMS,
                            new String[]{ItemsContract.ItemsColumns._ID},
                            ItemsContract.ItemsColumns.SERVER_ID + "=?",
                            new String[]{serverId},
                            null, // groupBy
                            null, // having
	...
	private SelectionBuilder buildSelection(Uri uri, int match, SelectionBuilder builder) {
		...
		switch (match) {
            ...
            case ITEMS__SERVER_ID: {
                final String serverId = paths.get(1);
                return builder.table(Tables.ITEMS)
                        .where(ItemsContract.Items.SERVER_ID + "=?", serverId);
            }
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
